import React from "react";

const Navbar = () => {
  return (
    <div className="flex justify-between items-center px-12 p-4 text-sm font-bold">
      <div>
        <img
          className="h-3 invert"
          src="data:image/svg+xml;base64,"
          alt="Tesla logo"
        />
      </div>

      <div className="hidden lg:inline">
        <ul className="flex justify-center hover:cursor-pointer">
          <li className="px-3 py-1 hover:rounded hover:bg-black/5 text-white">Model S</li>
          <li className="px-3 py-1 hover:rounded hover:bg-black/5 text-white">Model 3</li>
          <li className="px-3 py-1 hover:rounded hover:bg-black/5 text-white">Model X</li>
          <li className="px-3 py-1 hover:rounded hover:bg-black/5 text-white">Model Y</li>
          <li className="px-3 py-1 hover:rounded hover:bg-black/5 text-white">Solar Roof</li>
          <li className="px-3 py-1 hover:rounded hover:bg-black/5 text-white">Solar Panels</li>
          <li className="px-3 py-1 hover:rounded hover:bg-black/5 text-white">Powerwall</li>
        </ul>
      </div>

      <div className="hidden lg:inline">
        <ul className="flex justify-center hover:cursor-pointer">
          <li className="px-3 py-1 hover:rounded hover:bg-black/5 text-white">Shop</li>
          <li className="px-3 py-1 hover:rounded hover:bg-black/5 text-white">Account</li>
          <li className="px-3 py-1 hover:rounded hover:bg-black/5 text-white">Menu</li>
        </ul>
      </div>
      <div className="lg:hidden">
        <button className="inline-flex items-center rounded-md py-2 px-4 text-sm font-medium bg-black/5 shadow-sm hover:bg-black/10 text-white">Menu</button>
      </div>
    </div>
  );
};

export default Navbar;
