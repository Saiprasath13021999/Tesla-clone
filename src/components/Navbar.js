import React from "react";

const Navbar = () => {
  return (
    <div className="flex justify-between items-center px-12 p-4 text-sm font-bold">
      <div>
        <img
          className="h-3"
          src="data:image/svg+xml;base64,"
          alt="Tesla logo"
        />
      </div>

      <div className="hidden lg:inline">
        <ul className="flex justify-center hover:cursor-pointer">
          <li className="px-3 py-1 hover:rounded hover:bg-black/5">Model S</li>
          <li className="px-3 py-1 hover:rounded hover:bg-black/5">Model 3</li>
          <li className="px-3 py-1 hover:rounded hover:bg-black/5">Model X</li>
          <li className="px-3 py-1 hover:rounded hover:bg-black/5">Model Y</li>
          <li className="px-3 py-1 hover:rounded hover:bg-black/5">Solar Roof</li>
          <li className="px-3 py-1 hover:rounded hover:bg-black/5">Solar Panels</li>
          <li className="px-3 py-1 hover:rounded hover:bg-black/5">Powerwall</li>
        </ul>
      </div>

      <div className="hidden lg:inline">
        <ul className="flex justify-center hover:cursor-pointer">
          <li className="px-3 py-1 hover:rounded hover:bg-black/5">Shop</li>
          <li className="px-3 py-1 hover:rounded hover:bg-black/5">Account</li>
          <li className="px-3 py-1 hover:rounded hover:bg-black/5">Menu</li>
        </ul>
      </div>
      <div className="lg:hidden">
        <button className="inline-flex items-center rounded-md py-2 px-4 text-sm font-medium bg-black/5 shadow-sm hover:bg-black/10">Menu</button>
      </div>
    </div>
  );
};

export default Navbar;
